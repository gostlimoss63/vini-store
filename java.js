...
(() => {
  ...
  const FRAGS = ['promotion_bar','header','slider','categories','product_grid','footer'];
  ...
  const jobs = FRAGS.map(name => fetchText(`${BASE}${name}.html`).then(html => ({name, html})));
  const settled = await Promise.allSettled(jobs);
  settled.forEach(s => {
    if (s.status !== 'fulfilled') return console.warn('fragment job failed', s);
    const {name, html} = s.value;
    const slot = id(name);
    if (slot) slot.innerHTML = (html && html.trim()) ? html : `<!-- ${name} empty -->`;
...
function renderProducts(filter = 'all') {
  const productGrid = document.querySelector('.product_grid .products-container');
  if (!productGrid) {
  ...
  const filteredProducts = filter === 'all' 
    ? products 
    : products.filter(product => product.tab === filter);
...
function initProductGrid() {
  ...
  if (sortSelect) {
    sortSelect.addEventListener('change', function() {
      ...
      const activeTab = document.querySelector('.product_grid .tab-btn.active');
  ...
  function initMobileMenu() {
    const btn = id('btn-mobile'), menu = id('mobile-menu');
    if (!btn || !menu) return;
    ...
    menu.addEventListener('click', e => {
      if (e.target && e.target.tagName === 'A') { btn.setAttribute('aria-expanded','false'); menu.setAttribute('hidden',''); }
    }, {passive:true});
  }
  ...
  function initSlider() {
    ...
    const prev = id('prev'), next = id('next'), dots = id('dots');
    const slides = Array.from(slidesEl.children).filter(n=>n.nodeType===1);
    const N = slides.length; if (N===0) return;
    ...
    const go = (i, instant) => {
      ...
      cur = t;
      if (instant) requestAnimationFrame(()=>requestAnimationFrame(()=> slidesEl.style.transition = 'transform .45s ease'));
      updateUI();
    };
    const nextFn = ()=> go(cur+1), prevFn = ()=> go(cur-1);
    ...
    const buildDots = ()=> {
      ...
      for (let i=0;i<N;i++) {
        ...
        b.type='button'; b.dataset.i=i; b.ariaLabel = `Ir para slide ${i+1}`;
    ...
    const updateUI = ()=>{
      ...
      if (dots) Array.from(dots.children).forEach((b,idx)=> idx===cur ? b.setAttribute('aria-current','true') : b.removeAttribute('aria-current'));
    };

    if (prev) prev.addEventListener('click', ()=>{ prevFn(); resetAutoplay(); });
    if (next) next.addEventListener('click', ()=>{ nextFn(); resetAutoplay(); });
    window.addEventListener('keydown', e=> { if (e.key==='ArrowLeft') { prevFn(); resetAutoplay(); } if (e.key==='ArrowRight') { nextFn(); resetAutoplay(); } });

    // swipe
    let sx=0, dx=0; const TH=30;
    wrapper.addEventListener('touchstart', e=> { if (e.touches.length!==1) return; sx=e.touches[0].clientX; dx=0; slidesEl.style.transition='none'; }, {passive:true});
    wrapper.addEventListener('touchmove', e=> { if (e.touches.length!==1) return; dx = e.touches[0].clientX - sx; slidesEl.style.transform = `translateX(${-(cur * pct()) + (dx/(wrapper.clientWidth||1))*pct()}%)`; }, {passive:true});
    wrapper.addEventListener('touchend', ()=> { slidesEl.style.transition='transform .45s ease'; if (Math.abs(dx) > TH) dx < 0 ? nextFn() : prevFn(); else go(cur); resetAutoplay(); });

    // autoplay
    const startAutoplay = ()=> { if (!AUTOPLAY_MS || N<=1) return; stopAutoplay(); at = setInterval(()=> cur < N-1 ? nextFn() : go(0), AUTOPLAY_MS); };
    const stopAutoplay = ()=> { if (at) { clearInterval(at); at=null; } };
    const resetAutoplay = ()=> { stopAutoplay(); startAutoplay(); };

    // resize debounce
    let rt=null; window.addEventListener('resize', ()=> { if (rt) clearTimeout(rt); rt = setTimeout(()=>{ applyWidths(); go(cur,true); },120); });

    applyWidths(); buildDots(); go(0,true); startAutoplay();
  }

  // init
  async function init() {
    await Promise.allSettled([loadFragments()]);
    await Promise.resolve(); // allow DOM parse
    try { initMobileMenu(); } catch(e) { console.error('mobile init', e); }
    try { initSlider(); } catch(e) { console.error('slider init', e); }
  }
  ...
  setTimeout(()=> {
    const empty = FRAGS.every(n => { const el = id(n); return el && el.innerHTML.trim().length === 0; });
    if (empty) {
      console.warn('Fragments empty — if you opened index.html via file:// the browser may block fetch. Run a local server (e.g. python -m http.server).');
      const note = document.createElement('div');
      note.textContent = 'Fragments failed to load — see console';
      Object.assign(note.style, {position:'fixed',left:8,bottom:8,padding:'8px 12px',background:'rgba(0,0,0,.75)',color:'#fff',zIndex:9999,borderRadius:'6px',fontSize:'13px'});
...
function debugProductGrid() {
  ...
  console.log('Products container:', document.querySelector('.product_grid .products-container'));
